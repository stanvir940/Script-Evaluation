...
async function buildModerationCase(adjudicationId: string): Promise<ModerationCaseDto | null> {
  ...
  const evalDetails = await Promise.all(
    evaluations.map(async (ev) => {
      ...
      const layer = await AnnotationLayer.findOne({ answerId: answer._id, teacherId: ev.teacherId });
      const annotations: AnnotationAction[] = (layer?.actions ?? []).map((a) => ({
        id: a.id,
        tool: a.tool,
        points: a.points,
        color: a.color,
...
router.get('/next', asyncHandler(async (_req: AuthRequest, res: Response) => {
  const adj = await Adjudication.findOne({ status: 'PENDING' }).sort({ createdAt: 1 });
  if (!adj) {
...
router.post('/:id/decide', validateBody(decideSchema), asyncHandler(async (req: AuthRequest, res: Response) => {
  ...
  await writeAuditFromRequest(req, 'ADJUDICATION_COMPLETED', 'Adjudication', adjudication._id.toString());

  const nextAdj = await Adjudication.findOne({ status: 'PENDING' }).sort({ createdAt: 1 });
  const next = nextAdj ? await buildModerationCase(nextAdj._id.toString()) : null;
