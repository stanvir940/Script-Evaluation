...
import { EvaluationWorkspaceDto, AnnotationAction, TeacherStatsDto } from '@dasems/shared-types';
  ...
  return (
    <div className="h-full overflow-y-auto bg-white border-r border-border p-4 w-full md:w-[380px] shrink-0">
      <div className="mb-4 pb-4 border-b border-border">
        ...
        <p className="text-sm text-text-muted">Max marks: {workspace.maxMarks}</p>
      </div>
      <section className="mb-4">
        <h3 className="text-lg font-semibold mb-2">Question</h3>
        <p className="text-base leading-relaxed whitespace-pre-wrap">{workspace.questionText}</p>
      </section>
      <section className="mb-4">
        <h3 className="text-lg font-semibold mb-2">Rubric</h3>
        <p className="text-base text-text-secondary whitespace-pre-wrap">{workspace.rubric}</p>
      </section>
      <section className="mb-4">
      ...
      {workspace.keywords.length > 0 && (
        <section>
          ...
          <p className="text-sm text-text-muted">{workspace.keywords.join(', ')}</p>
        </section>
      )}
    </div>
  );
...
export function EvaluationPage() {
  const [workspace, setWorkspace] = useState<EvaluationWorkspaceDto | null>(null);
  ...
  const loadNext = useCallback(async () => {
    ...
    setError('');
    try {
      const data = await api.get<EvaluationWorkspaceDto | null>('/evaluations/next');
      setWorkspace(data);
  ...
  const saveDraft = useCallback(async () => {
    if (!workspace) return;
    try {
      await api.post(`/evaluations/assignments/${workspace.assignmentId}/save`, {
        mark: mark === '' ? undefined : parseFloat(mark),
        comment,
      });
      await api.post(`/evaluations/assignments/${workspace.assignmentId}/annotations`, { actions: annotations });
    } catch {
      /* silent autosave */
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post(`/evaluations/assignments/${workspace.assignmentId}/annotations`, { actions: annotations });
      const result = await api.post<{ submitted: boolean; next: EvaluationWorkspaceDto | null }>(
    ...
    <AppLayout>
      <div className="flex flex-col h-[calc(100vh-56px)]">
        <div className="flex items-center justify-between px-6 py-2 bg-white border-b border-border">
          <Link to="/" className="text-sm text-text-secondary hover:text-primary">← Exit</Link>
          {workspace && (
            <span className="text-sm text-text-muted">
              {workspace.progress.completed}/{workspace.progress.assigned} completed
            </span>
          )}
        </div>
        ...
        {!loading && workspace && (
          <>
            ...
            >
              <div className="w-32">
                <label className="block text-sm font-medium mb-1">Mark / {workspace.maxMarks}</label>
                <input
                  type="number"
                  step="0.5"
                  min={0}
              ...
              <div className="flex-1 min-w-[200px]">
                ...
                <input value={comment} onChange={(e) => setComment(e.target.value)} className="input-field" />
              </div>
              <button type="button" onClick={saveDraft} className="btn-secondary">Save</button>
      ...
      <div className="p-6 max-w-4xl">
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            <div className="stat-box"><p className="stat-label">Assigned</p><p className="stat-value">{stats.assigned}</p></div>
            <div className="stat-box"><p className="stat-label">Completed</p><p className="stat-value text-success">{stats.completed}</p></div>
            <div className="stat-box"><p className="stat-label">Remaining</p><p className="stat-value">{stats.remaining}</p></div>
            <div className="stat-box"><p className="stat-label">Today</p><p className="stat-value">{stats.evaluatedToday}</p></div>
          </div>
        )}
        <Link to="/evaluate" className="btn-primary text-lg px-8 py-3 inline-flex">Start Evaluation</Link>
      </div>
    </AppLayout>
  );
}
