...
export function ModerationPage() {
  ...
  const loadNext = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.get<ModerationCaseDto | null>(
        "/adjudications/next",
      );
      setCaseData(data);
      setFinalMark("");
        ...
        <div className="px-6 py-2 bg-white border-b border-border flex justify-between items-center">
          ...
          {caseData && (
            <span className="badge-danger">
              Spread: {caseData.markSpread} marks
            </span>
          )}
        </div>
            ...
            <div className="flex flex-1 overflow-hidden flex-col lg:flex-row">
              <div className="w-full lg:w-[320px] p-4 bg-white border-r border-border overflow-y-auto shrink-0">
                <h2 className="text-lg font-semibold">
                  Q{caseData.questionNumber} — {caseData.subject.name}
                </h2>
                <p className="text-sm text-text-muted mb-4">
                  Script: {caseData.sCode}
                </p>
...
function TeacherSummaryTable({
  ...
  teachers: TeacherEvaluationSummaryDto[];
}) {
  return (
        ...
        <tbody>
          {teachers.map((teacher) => (
            <tr key={teacher.teacherId}>
              ...
              <td>{teacher.subjectCodes.join(", ") || "—"}</td>
              <td>{teacher.assigned}</td>
              <td>{teacher.inProgress}</td>
              <td>{teacher.submitted}</td>
              <td>{teacher.remaining}</td>
              <td>
                {teacher.recentEvaluations.length === 0 && "—"}
                {teacher.recentEvaluations.map((ev) => (
                  <div key={ev.answerId} className="text-xs mb-1">
                    <strong>{ev.sCode}</strong> Q{ev.questionNumber}{" "}
                    {ev.subjectCode} • {ev.mark}
                  </div>
                ))}
              </td>
            </tr>
...
export function HeadExaminerDashboard() {
  const {
    ...
    queryFn: () =>
      api.get<TeacherEvaluationSummaryDto[]>("/results/teacher-summary"),
  });
    ...
    <AppLayout>
      <div className="p-6 max-w-6xl">
        ...
        <div className="panel p-6 mb-6">
          ...
          {isLoading && <p>Loading teacher summary…</p>}
          {error && (
            <p className="text-danger">Failed to load teacher summary.</p>
          )}
          {teachers && teachers.length === 0 && (
            <p>No active teachers found.</p>
          )}
          {teachers && teachers.length > 0 && (
            <TeacherSummaryTable teachers={teachers} />
          )}
        </div>
      </div>
    </AppLayout>
